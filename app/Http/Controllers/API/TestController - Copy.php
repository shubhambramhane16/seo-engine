...
class TestController extends Controller
{
    ...
    public function generateSignature($request, $queryParams = [])
    {
        ...
        $cachedToken = Cache::get('guest_token');

        if ($cachedToken) {
            $xAuthToken = $cachedToken;
        } else {
            $authController = new AuthController();
            $tokenResponse = $authController->loginbyGuest($request);
            $tokenData = json_decode($tokenResponse, true);

            if (isset($tokenData['data']['token'])) {
                $xAuthToken = $tokenData['data']['token'];

                // Decode JWT to get expiration time
                $tokenParts = explode('.', $xAuthToken);
                if (count($tokenParts) === 3) {
                    $payload = json_decode(base64_decode($tokenParts[1]), true);
                    if (isset($payload['exp'])) {
                        $expiresAt = Carbon::createFromTimestamp($payload['exp']);

                        // Cache token until its expiration time (or max 1 hour)
                        $cacheTime = min($expiresAt, Carbon::now()->addHour());
                        Cache::put('guest_token', $xAuthToken, $cacheTime);
                    }
                }
            } else {
    ...
    public function getTestbyItemId(Request $request, $city_id, $slug_name)
    {
        ...
        try {
            ...
            if (isset($preLoginScript['x-signature']) && !empty($preLoginScript['x-signature']) && isset($preLoginScript['x-nonce']) && !empty($preLoginScript['x-signature'])) {
                ...
                $accept = env('ACCEPT');
                $xLanguage = 'en';//env('X_LANGUAGE') ?? 'en';
