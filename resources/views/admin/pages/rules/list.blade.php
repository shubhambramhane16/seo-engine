...
<div class="card card-custom">
    <div class="card-header flex-wrap border-0 pt-3 pb-0">
        ...
        <div class="card-toolbar">
            ...
            <!--begin::Button-->
            <a href="{{url('/admin/rules/add')}}" class="btn btn-primary font-weight-bolder">
        ...
        <table class="table table-bordered table-hover" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $value->template?->template_name }}</td>
                    <td>{{ $value->rule_name }}</td>
                    <td>{{ $value->prefix }}</td>
                    <td>{{ $value->url_structure }}</td>
                    <td>
                        <div class="temp-act-inact-btn">
                            <div class="temp-active">
                                <a href="javascript:void(0)" data-url="{{url('admin/rules/update-status/'.$value->id.'/'.$value->status)}}" onclick="changeStatus(this)"> <span class="label label-lg font-weight-bold label-light-{{($value->status == 1) ? 'success' : 'danger'}} label-inline">{{($value->status == 1) ? 'Active' : 'InActive'}}</span></a>
                            </div>
                        </div>
                    </td>
                    <td>
                        <a href="{{ url('admin/rules/edit/'.$value->id) }}" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details"> <i class="la la-edit"></i> </a>
