...
@section('localitymaster','active menu-item-open')
@section('content')
    ...
    <div class="card-header flex-wrap border-0 pt-3 pb-0">
        ...
        <div class="card-toolbar">
            @include('admin.layout.partials.filters.common-filter')
             <!--begin::Button-->
             <a href="{{url('/admin/locality/import')}}" class="btn btn-light-primary font-weight-bolder">
            ...
            &nbsp;
            <a href="{{url('/admin/locality/sync')}}" class="btn btn-primary font-weight-bolder">
                <i class="la la-sync"></i>Sync</a>
            &nbsp;

            <a href="{{url('/admin/locality/add/'.$stateId.'/'.$cityId)}}" class="btn btn-primary font-weight-bolder">
        ...
        <table class="table table-bordered table-hover" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{$value->slug}}</td>
                    <td>
                        <a href="javascript:void(0)" data-url="{{url('admin/locality/update-status/'.$value->id.'/'.$value->status)}}" onclick="changeStatus(this)"> <span class="label label-lg font-weight-bold label-light-{{($value->status == 1) ? 'success' : 'danger'}} label-inline">{{($value->status == 1) ? 'Active' : 'InActive'}}</span></a>
                    </td>

                    <td>
                        <a href="{{url('/admin/locality/edit/'.$value->state?->id.'/'.$value->city?->id.'/'.$value->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit details" data-toggle="tooltip"> <i class="la la-edit"></i> </a>
                    </td>
                </tr>
                @endforeach
                @endif
