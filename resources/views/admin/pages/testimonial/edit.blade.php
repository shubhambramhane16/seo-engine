                                        ...
                                        <select name="city_id" id="cities" isrequired="isrequired"
                                            ...
                                            @foreach ($cities as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $testimonialDetail->city_id == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        <select name="locality_id" id="locality" isrequired="isrequired"
                                            ...
                                            @foreach ($localities as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $testimonialDetail->locality_id == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        <select name="centre_id" id="locality" isrequired="isrequired"
                                            ...
                                            @foreach ($centeres as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $testimonialDetail->centre_id == $item->id ? 'selected' : '' }}>
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <div>
                                        <textarea type="text" name="content" placeholder="Write Content.." rows="8" class="form-control"
                                            isrequired="isrequired">{{ $testimonialDetail->content }}</textarea>
                                    </div>
                                </div>
                                <div class="form-group col-md-12">
                                    <label>Rating</label>
                                    <div class="rating">
                                        @for ($i=5; $i>=1;$i--)
                                         <input type="radio" id="star{{$i}}" name="rating" value="{{$i}}" required {{ $testimonialDetail->rating == $i ? 'checked' : ''}}><label
                                        for="star{{$i}}"></label>
                                        @endfor
                                    </div>
                                </div>
