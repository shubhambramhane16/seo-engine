...
<div class="card card-custom">
    <div class="card-header flex-wrap border-0 pt-3 pb-0">
        <div class="card-title">
            <h3 class="card-label">State List
            </h3>
        </div>
        <div class="card-toolbar">
            @include('admin.layout.partials.filters.common-filter')
            <!--begin::Button-->
            <a href="{{url('/admin/state/import')}}" class="btn btn-light-primary font-weight-bolder">
            ...
            &nbsp;
            <a href="{{url('/admin/state/sync')}}" class="btn btn-primary font-weight-bolder">
                <i class="la la-sync"></i>Sync</a>
            &nbsp;

            <a href="{{url('/admin/state/add')}}" class="btn btn-primary font-weight-bolder">
                <i class="la la-plus"></i>Add State</a>
            <!--end::Button-->
        </div>
            ...
            <div class="row col-lg-12 pl-0 pr-0">
                ...
                <div class="col-sm-5">
                    <div class="dataTables_length">
                        ...
                        <a href="{{url('admin/state/list')}}" class="btn btn-default" data-toggle="tooltip" title="Reset Filter" style="margin-top: 20px;">Reset</a>
                    </div>
                </div>
        ...
        <table class="table table-bordered table-hover" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{$value->slug}}</td>
                    <td>
                        <a href="javascript:void(0)" data-url="{{url('admin/state/update-status/'.$value->id.'/'.$value->status)}}" onclick="changeStatus(this)"> <span class="label label-lg font-weight-bold label-light-{{($value->status == 1) ? 'success' : 'danger'}} label-inline">{{($value->status == 1) ? 'Active' : 'InActive'}}</span></a>
                    </td>
                    <td>
                        <a href="{{url('/admin/state/edit/'.$value->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit details" data-toggle="tooltip"> <i class="la la-edit"></i> </a>
                    </td>
                </tr>
                @endforeach
                @endif
