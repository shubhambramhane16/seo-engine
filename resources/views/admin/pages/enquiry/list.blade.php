...
<div class="card card-custom">
    <div class="card-header flex-wrap border-0 pt-3 pb-0">
        <div class="card-title">
            <h3 class="card-label">Enquiry List
            </h3>
        </div>
            ...
            <div class="row col-lg-12 pl-0 pr-0">
                ...
                <div class="col-sm-5">
                    <div class="dataTables_length">
                        ...
                        <a href="{{url('admin/enquiry/list')}}" class="btn btn-default" data-toggle="tooltip" title="Reset Filter" style="margin-top: 20px;">Reset</a>
                    </div>
                </div>
        ...
        <table class="table table-bordered table-hover" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                   ...
                   <td>{{$value->slot_time}}</td> --}}
                   <td>{{$value->city}}</td>
                   <td>{{$value->locality}}</td>
                   {{-- <td>{{$value->item_id}}</td> --}}
                   <td>{{$value->item_reference}}</td>
                   <td>{{$value->form}}</td>
                   <td>{{$value->query}}</td>
                    <td>
                        <a href="javascript:void(0)" data-url="{{url('admin/enquiry/update-status/'.$value->id.'/'.$value->status)}}" onclick="changeStatus(this)"> <span class="label label-lg font-weight-bold label-light-{{($value->status == 1) ? 'success' : 'danger'}} label-inline">{{($value->status == 1) ? 'Active' : 'InActive'}}</span></a>
                    </td>
                    <td>
                        <a href="{{url('/admin/enquiry/edit/'.$value->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit details" data-toggle="tooltip"> <i class="la la-edit"></i> </a>
                    </td>
                </tr>
                @endforeach
                @endif
